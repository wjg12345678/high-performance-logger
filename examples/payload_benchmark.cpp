...
void PrintImprovement(
    ...
    const LatencySummary& optimized) {
  if (baseline.median_mean_ns > 0.0) {
    const double mean_improvement =
        (baseline.median_mean_ns - optimized.median_mean_ns) / baseline.median_mean_ns * 100.0;
    std::cout << name << "_mean_improvement_percent=" << mean_improvement << '\n';
  }
  if (baseline.median_p99_ns > 0) {
    const double p99_improvement =
        (static_cast<double>(baseline.median_p99_ns) - static_cast<double>(optimized.median_p99_ns)) /
        static_cast<double>(baseline.median_p99_ns) * 100.0;
    std::cout << name << "_p99_improvement_percent=" << p99_improvement << '\n';
  }
}
...
int main(int argc, char** argv) {
  ...
  std::cout << "config threads=" << thread_count
            ...
            << " long_payload_bytes=" << hlog_bench::LongPayloadBlob().size()
            << '\n';

  const auto factory = []() {
    hlog::AsyncLoggerOptions options;
    options.queue_size = 1 << 18;
  ...
  const auto flush = [](hlog::AsyncLogger& logger) {
    logger.Flush();
  };

  const auto prebuilt_short = [](hlog::AsyncLogger& logger, int thread_id, int index) {
    return logger.Info(hlog_bench::MakeStreamPayload(thread_id, index));
  };
  const auto variadic_short = [](hlog::AsyncLogger& logger, int thread_id, int index) {
    return logger.Info("thread=", thread_id, " seq=", index);
  };
  const auto prebuilt_long = [](hlog::AsyncLogger& logger, int thread_id, int index) {
    return logger.Info(hlog_bench::MakeStreamLargePayload(thread_id, index));
  };
  const auto variadic_long = [](hlog::AsyncLogger& logger, int thread_id, int index) {
    return logger.Info(
        "thread=",
        thread_id,
        " seq=",
  ...
  const auto prebuilt_short_results = RunMeasuredRounds(
      measured_rounds,
      "prebuilt_short",
      thread_count,
      messages_per_thread,
      ...
      prebuilt_short,
      flush);
  const auto variadic_short_results = RunMeasuredRounds(
      measured_rounds,
      "variadic_short",
      thread_count,
      messages_per_thread,
      ...
      variadic_short,
      flush);
  const auto prebuilt_long_results = RunMeasuredRounds(
      measured_rounds,
      "prebuilt_long",
      thread_count,
      messages_per_thread,
      ...
      prebuilt_long,
      flush);
  const auto variadic_long_results = RunMeasuredRounds(
      measured_rounds,
      "variadic_long",
      thread_count,
      messages_per_thread,
  ...
  const auto prebuilt_short_summary = Summarize(prebuilt_short_results);
  const auto variadic_short_summary = Summarize(variadic_short_results);
  const auto prebuilt_long_summary = Summarize(prebuilt_long_results);
  const auto variadic_long_summary = Summarize(variadic_long_results);

  PrintSummary(prebuilt_short_summary);
  PrintSummary(variadic_short_summary);
  PrintSummary(prebuilt_long_summary);
  PrintSummary(variadic_long_summary);

  PrintImprovement("short", prebuilt_short_summary, variadic_short_summary);
  PrintImprovement("long", prebuilt_long_summary, variadic_long_summary);
  return 0;
}
